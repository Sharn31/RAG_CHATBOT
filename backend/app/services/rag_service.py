...
4. Deterministic experience/tenure calculation (resumes only)
5. Semantic retrieval
6. LLM-powered answer generation
7. Conversation context management

DESIGN NOTE ON THE SHORTCUTS BELOW: pure semantic search already handles
general document Q&A well (skills, education, projects, company details,
anything with real semantic content). It is specifically weak at literal
identity fields - a person's name has almost no semantic meaning for an
embedding model to match against ("Prabhjot Singh" and "John Smith" embed
nearly identically). The identity shortcut and experience calculator exist
to cover exactly that narrow gap, not to replace semantic search generally.
Both are no-ops on non-resume documents and fall straight through to normal
retrieval, so uploading a research paper, contract, or anything else behaves
as plain RAG with no special-casing applied.
"""
...
class DocumentType(Enum):
    """Classify document to decide whether resume-only shortcuts apply."""
    RESUME = "resume"
    RESEARCH = "research"
    LEGAL = "legal"
    GENERAL = "general"
...
def detect_document_type(first_chunks: List[Dict]) -> DocumentType:
    """Detect document type from the first couple of chunks."""
    text = " ".join(c.get("text", "")[:500] for c in first_chunks[:2]).lower()
...
class ResumeEntityExtractor:
    """Extract key resume entities to answer identity questions directly,
    bypassing semantic search for the one field type it's structurally weak at.
    """

    @staticmethod
    def extract_name(text: str) -> Optional[str]:
        """Extract full name from resume - handles three header styles:
        clean multi-line, whole-line ALL CAPS, and dense single-line headers
        where name/title/contact are all run together (e.g. "PRABHJOT SINGH
        React.js | Next.js | Node.js Developer prabhjotchd001@gmail.com...").
        """
        lines = text.split('\n')
        for line in lines[:3]:
            line = line.strip()
            # Whole line is just the name, Title Case: "Jasbir Singh"
            if re.match(r'^[A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3}$', line):
                return line
            # Whole line is just the name, ALL CAPS: "JASBIR SINGH"
            if re.match(r'^[A-Z]+(?:\s+[A-Z]+){1,3}$', line) and len(line.split()) <= 4:
                return line.title()
            # Name is the LEADING portion of a dense single-line header:
            # match 2-4 ALL CAPS words at the very start of the line.
            match = re.match(r'^([A-Z]{2,}(?:\s+[A-Z]{2,}){1,3})\b', line)
            if match:
    ...
    def extract_linkedin(text: str) -> Optional[str]:
        """Extract LinkedIn profile URL, with or without the https:// scheme."""
        match = re.search(r'(?:https?://)?(?:www\.)?linkedin\.com/in/[A-Za-z0-9\-_%]+', text, re.IGNORECASE)
        return match.group(0) if match else None

    @staticmethod
    def extract_entities(chunks: List[Dict]) -> Dict[str, str]:
        """Extract all resume entities from the first few chunks."""
        combined_text = " ".join(c.get("text", "") for c in chunks[:5])
        ...
        return {
            ...
            "email": ResumeEntityExtractor.extract_email(combined_text),
            "phone": ResumeEntityExtractor.extract_phone(combined_text),
            "linkedin": ResumeEntityExtractor.extract_linkedin(combined_text),
        }
...
class ExperienceCalculator:
    """Computes total work experience deterministically from date ranges
    found in the document, instead of asking the LLM to do date arithmetic
    on raw text (which it does unreliably and inconsistently between runs).

    Gated to RESUME documents only (see is_total_experience_question call
    site in answer_question) - a research paper or contract can contain date
    ranges too (study periods, contract terms), which aren't "experience"
    and shouldn't be summed as such.
    """
    ...
    @staticmethod
    def compute_total(chunks: List[Dict]) -> Optional[str]:
        """Sums non-overlapping, non-education date ranges found across all
        chunks. Returns None if no parseable ranges are found, so the caller
...
class QueryOptimizer:
    ...
    # Keyed by topic, matched by individual trigger WORDS present anywhere in
    # the question (checked BEFORE pronoun stripping, since trigger phrases
    # like "my experience" would partially disappear if "my" were stripped
    # first).
    TOPIC_EXPANSIONS = {
        "name":       (["name"],                        ["name", "full name", "called"]),
        "email":      (["email"],                        ["email", "email address", "contact"]),
    ...
    @staticmethod
    def expand_short_question(question: str) -> str:
        """Expand vague/short questions by adding related terms - checked
        against whole words, not substrings (avoids "i" falsely matching
        inside words like "skills").
        """
        question_lower = question.lower().strip('?').strip()
        words = set(question_lower.split())
    ...
    @staticmethod
    def optimize(question: str, document_type: DocumentType) -> str:
        """Main optimization pipeline. Expansion runs BEFORE pronoun removal
        (see TOPIC_EXPANSIONS docstring above for why the order matters).
        """
        query = question
...
class MultiLayerRetrieval:
    ...
    def hybrid_search(
        ...
        `entities` is accepted for call-site compatibility but no longer
        used to bias scoring: entity-boosting was removed because it
        permanently over-ranked whichever chunk happened to contain contact
        info, crowding out topically relevant chunks for unrelated questions.
        """
        store = vector_service.get_vector_store()
        query_embedding = embedding_service.embed_query(question)
        results = store.query_session(session_id, query_embedding, top_k)
...
IDENTITY_QUESTION_PATTERNS = {
    ...
    "email": ["what is my email", "whats my email", "my email"],
    "phone": ["what is my phone", "whats my phone", "my phone number", "my number"],
    "linkedin": ["what is my linkedin", "whats my linkedin", "my linkedin"],
}


def match_identity_question(question: str) -> Optional[str]:
    """Detects a direct identity question and returns which extracted
    entity key it maps to, or None if it's not one. See module docstring
    for why this exists (embeddings are structurally weak at literal name
    matching, unlike everything else in a document).
    """
    q = question.lower().strip().strip('?').strip()
    for entity_key, patterns in IDENTITY_QUESTION_PATTERNS.items():
        if any(p in q for p in patterns):
...
def answer_question(session_id: str, question: str) -> ChatResponse:
    ...
    through to normal retrieval + LLM; both are no-ops on non-resume
    documents (empty entities dict / doc_type check), so any other PDF type
    behaves as plain semantic-search RAG with no special-casing applied.
    """
    ...
    # Fast path 2: total-experience questions, answered via deterministic
    # date math. Explicitly gated to RESUME documents - a research paper or
    # contract can contain date ranges too (study periods, contract terms),
    # which aren't "experience" and shouldn't be summed as such.
    if doc_type == DocumentType.RESUME and ExperienceCalculator.is_total_experience_question(question):
        all_chunks = chunk_service.get_cached_chunks(session_id)
        total = ExperienceCalculator.compute_total(all_chunks)
        if total:
            answer_text = (
    ...
    # LLM only ever accepts {role, content} per message - strip the
    # "timestamp" field memory_service stores internally, or strict
    # providers (Groq in particular) reject the request outright.
    raw_history = memory.get_history(session_id)
    history = [{"role": h["role"], "content": h["content"]} for h in raw_history]
